...
// 12 - nextFillDate (uint256)
    ...
    // const hashedName = ethers.utils.keccak256(ethers.utils.toUtf8Bytes(name));
    // const hashedDob = ethers.utils.keccak256(ethers.utils.toUtf8Bytes(dob));
    // const hashed_pt_physical_address = ethers.utils.keccak256(ethers.utils.toUtf8Bytes(pt_physical_address));
    ...
    const data = await _createScript({ args: [wallet_address, hashedName, description_Ref, medication_Ref, hashedDob, 
      hashed_pt_physical_address, quantity_Ref, doctor, dea, new Date(date_prescribed_Ref).getTime(), perDiemMax, 
      // new Date(nextFillDate).getTime()
      new Date(nextFillDate).getTime()
    ] })
